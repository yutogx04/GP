...
Before running the backend, you must create your own private `.env` file.
...
If it redirects to `/login` (or `/dashboard` if logged in), the routing (Vue Router) and State Management (Pinia) are working.

### 4. API Connectivity
The Frontend relies on the Backend running at port 8000.  
If you see "Network Error" or infinite loading spinners, ensure the backend terminal is running and no firewall is blocking `localhost:8000`.
